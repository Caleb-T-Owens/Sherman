...
canonicalUrl: https://cto.je/minecraft/beta-minecraft/playing-beta-minecraft
description: Adventures in Minecraft Beta.
---
...
<LastUpdated filePath="src/pages/minecraft/beta-minecraft/playing-beta-minecraft.mdx" />

Playing a
[14 year old version of a game](https://minecraft.wiki/w/Java_Edition_Beta_1.7.3)
comes with it's own challanges. Credit where it's due, Minecraft has rotted
pretty gracefully. As far as I'm aware - if you run beta 1.7.3 on windows or
linux with Java 1.8, it will run fine. On macOS it's a little more tricky. If
you start up the game, you will get a screenshot like this with the colors
inverted:

![Minecraft, but red and blue channels are swapped making the sky, water red, and a blue heath bar](./bgr-vs-rgb.png)

This
[_seems_ to be an endienness issue](https://www.minecraftforum.net/forums/support/java-edition-support/3049963-red-and-blue-colors-seem-to-be-inverted).
On little endien machines (like aarch64 macs), lwjgl 2 seems to expect bgr
colors instead of rgb. I'm not entirly sure why, and the existing fixes don't
come with any explanation about how their fixes actually solve the problem.

Before I talk about fixing the problem, the first thing we really need to do is
to figgure out how we want to launch the game. We need to modify the game's jar
file in order to run any modifications - or a inverted color fix. In the old
...
original authors. I ended up using Prism because it seems much more activly
...
![Prism launcher's "version" tab](./version-tab.png)

The first fix comes in the form of (https://betacraft.uk)'s
...
Actually modding beta Minecraft is something that has changed quite a bit in the
last couple years.
...
consumers of a common API. Because each mod is an indipendant module, they often
...
[StationAPI](https://stationapi.wiki). Babric is based on the relativly new
[FabricMC](https://fabricmc.net) project which was started as a freash project
...
![The mods tab in the Prism launcher](./mods-tab.png)

## How I set up my game

There are a wide range of mods available for Babric & StationAPI that can
complelty change your game. I was quite keen on staying within the "spirit" of
...
- [Inventory Tweaks](https://modrinth.com/mod/inventorytweaks) - brings modern
  inventory management shortcuts as well as a few additional tweaks from it's
  modern namesake
...
[Antiuqe Atlas mod](https://modrinth.com/mod/antique-atlas-4) so I might
